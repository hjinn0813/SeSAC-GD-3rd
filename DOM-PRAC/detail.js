...
// window.location.search: 쿼리스트링을 추출해준다
console.log(window.location.search);

/* URLSearchParams: 많은 쿼리스트링을 쉽게 추출할 수 있게 도와준다
예를 들어 ?id=1&name=layla&page=3처럼 
id, name, page와 같이 많은 쿼리스트링을 전달할 경우
.get()을 사용해 쉽게 해당 키의 값을 추출 가능 */
const urlParam = new URLSearchParams(window.location.search);
const productId = urlParam.get('id'); // 쿼리스트링 중 아이디 키의 값을 추출
...
// 찾은 상품을 브라우저에 보여주기 (id가 없을 경우의 상황 조건문으로)
